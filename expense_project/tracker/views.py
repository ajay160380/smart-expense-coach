...
HINGLISH_NUM_MAP = {
    ...
    r'\bchar\b':      '4', r'\bpaanch\b':    '5', r'\bchhe\b':      '6',
    r'\bsaat\b':      '7', r'\baath\b':      '8', r'\bnau\b':       '9',
    r'\bdas\b':       '10', r'\bgyarah\b':   '11', r'\bbaarah\b':   '12',
    r'\bterah\b':     '13', r'\bchaudah\b':  '14', r'\bpandrah\b':  '15',
    r'\bsolah\b':     '16', r'\bsattrah\b':  '17', r'\baathaarah\b':'18',
    r'\bunees\b':     '19', r'\bbees\b':     '20', r'\bpaccheees\b':'25',
    r'\btees\b':      '30', r'\bchaalis\b':  '40', r'\bpackaas\b':  '50',
    r'\saath\b':      '60', r'\bsaattar\b':  '70', r'\bassee\b':    '80',
    r'\bnabbe\b':     '90', r'\bsau\b':      '00', r'\bsou\b':      '00',
    r'\bhazar\b':     '000', r'\bhajaar\b':  '000', r'\blakh\b':    '00000',
    r'\bkarod\b':     '0000000',
}
...
def dashboard(request: HttpRequest) -> HttpResponse:
    ...
    context = {
        "budget":               budget,
        "insight":              insight,
        "ai_main_tip":          ai_main_tip,   
        "ai_sub_tip":           ai_sub_tip,    
...
def add_expense(request: HttpRequest) -> HttpResponse:
    ...
    messages.success(request, "Paisa track ho gaya! 💸")
    return redirect("dashboard")
...
def voice_expense(request: HttpRequest) -> JsonResponse:
    ...
    if not incoming_phone:
        ...
        if not profile or not profile.user:
            logger.warning("Voice expense failed: unknown phone %s", incoming_phone)
            return JsonResponse({
                "status":  "error",
...
def api_savings_projection(request: HttpRequest) -> JsonResponse:
    ...
    months_needed = math.ceil(goal / monthly_savings)
...
def habit_warnings(request):
    ...
    if not incoming_phone:
        ...
        if not profile or not profile.user:
            logger.warning("Habit warnings failed: unknown phone %s", incoming_phone)
            return JsonResponse({
                "status":  "error",
                "message": "Bhai, pehle website par register karke apna WhatsApp number save karo! 🚫"
            }, status=404)
    ...
    try:
        ...
        logger.error("Habit warnings error: %s", e)
...
def whatsapp_summary(request):
    if request.method == "POST":
        try:
            ...
            try:
                ai_suggestion = get_ai_insight(user.id, all_expenses, budget, float(lifetime_spent))
            except Exception as e:
                logger.error(f"AI Insight fail hua: {e}")
